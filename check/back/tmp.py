...
def main():
    ...
    data_2 = np.random.rand(128, 3, 32, 32).astype(np.float32)

    target_cla_1 = np.random.randint(0, 10, (128, 1))
    target_cla_2 = np.random.randint(0, 10, (128, 1))

    dataloader_ext = [(data_1, target_cla_1), (data_2, target_cla_2)]
    # ----------------------------------------模型----------------------------------------
    ##torch ext
    opt = {'num_classes': 4, 'num_stages': 4, 'use_avg_on_conv3': False}
    model_pytorch = NetworkInNetwork.create_model(opt)
    ...
    opt = {'num_classes': 4, 'num_stages': 4, 'use_avg_on_conv3': False}
    model_paddle = ext.create_model(opt)
    ...
    optim_torch = torch.optim.SGD(params=model_pytorch.parameters(), lr=optim_params['lr'],
                                      momentum=optim_params['momentum'],
                                      nesterov=optim_params['nesterov'] if ('nesterov' in optim_params) else False,
                                      weight_decay=optim_params['weight_decay'])
    optim_paddle = paddle.optimizer.Momentum(parameters=model_paddle.parameters(),
    ...
    for ele in dataloader_ext:
        ...
        predicted = model_paddle(paddle_x, ['conv2'])
        ...
        predicted = model_pytorch(torch_x, ['conv2'])
