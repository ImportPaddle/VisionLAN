...
def get():
    ...
    net = state['network']
    torch.save(net, "./model_pytorch.pth")
...
def trans():
    ...
    paddle.save(paddle_dict, './model_paddle.pdparams')
...
def main():
    ...
    data_1 = np.random.rand(128, 3, 32, 32).astype(np.float32)

    ##torch ext
    optExtTorch = {'num_classes': 4, 'num_stages': 4, 'use_avg_on_conv3': False, 'out_feat_keys': ['conv2']}
    model_pytorch = NetworkInNetwork.create_model(optExtTorch)
    state = torch.load("./model_pytorch.pth")
    model_pytorch.load_state_dict(state)
    ...
    model_paddle = ext.create_model(optExtPaddle)
    state = paddle.load("./model_paddle.pdparams")
    model_paddle.set_state_dict(state)
    ...
    pytorch_res1 = torchRes(model_pytorch, data_1, out_feat_keys=out_feat_keys)
    pytorch_cla_res1 = torchRes(model_cla_pytorch, pytorch_res1)

    print(pytorch_cla_res1.shape)

    paddle_res1 = paddleRes(model_paddle, data_1, out_feat_keys=out_feat_keys)
    paddle_cla_res1 = paddleRes(model_cla_paddle, paddle_res1)

    reprod_log_1.add("model_ext1", pytorch_res1)
    reprod_log_1.add("model_cla1", pytorch_cla_res1)
    reprod_log_1.save("net_pytorch.npy")

    reprod_log_2.add("model_ext1", paddle_res1)
