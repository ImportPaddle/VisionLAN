...
from api import IntTensor, clip_grad_norm_
from utils import *
...
def generate_optimizer(model):
    if cfgs.global_cfgs['step'] != 'LF_2':
        out = paddle.optimizer.Adam(parameters=[{'params': model.parameters(), 'lr': cfgs.optimizer_cfgs['optimizer_0_args']['lr']}])
        ...
        out = paddle.optimizer.Adam(parameters=[{'params': filter(lambda p: id(p) == id_total, model.parameters()), 'lr': cfgs.optimizer_cfgs['optimizer_0_args']['lr']},
