...
from api import normal_, xavier_normal_
...
class MultiHeadAttention(nn.Layer):
    ...
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1):
        ...
        normal_(self.w_qs.weight, mean=0, std=np.sqrt(2.0 / (d_model + d_k)))
        normal_(self.w_ks.weight, mean=0, std=np.sqrt(2.0 / (d_model + d_k)))
        normal_(self.w_vs.weight, mean=0, std=np.sqrt(2.0 / (d_model + d_v)))
...
class PP_layer(nn.Layer):
    ...
    def forward(self, enc_output):
        reading_order = paddle.arange(self.character_len, dtype=paddle.int64)
